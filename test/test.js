    ...
    describe("LinkedList Methods", () => {
        ...
        describe('removeTail', () => {
            ...
            it('Should reassign the new tail\'s next pointer to null', () => {
                ...
                expect(linkedList.head.next.next).excluding(`[Circular]`).to.eql({
                    value: 'C',
                    next: null,
                    prev: {
                        value: 'B',
            ...
            it('Should return the removed tail node when removeTail is called', () => {
                linkedList.addToTail('A');
                linkedList.addToTail('B');
                expect(linkedList.removeTail()).to.eql({ value: 'B', next: null, prev: null });
            });
        ...
        describe('addToHead', () => {
            it('Should reassign the head pointer when new nodes are added to the head', () => {
                ...
                expect(linkedList.head).excluding(`[Circular]`).to.eql({
                    value: 'A',
                    next: { value: 'B', next: null, prev: `[Circular]` },
                    prev: null
                });
            ...
            it('Should return the updated list after new nodes are added to the head', () => {
                ...
                expect(linkedList.addToHead('A')).excluding(`[Circular]`).to.eql({
                    head: {
                        value: 'A',
                        next: { value: 'B', next: null, prev: `[Circular]` },
                        prev: null
        ...
        describe('get', () => {
            ...
            it('Should return node at index specified when get is called', () => {
                ...
                linkedList.addToTail('B');
                linkedList.addToTail('C');
                expect(linkedList.get(1)).excluding(`[Circular]`).to.eql({
                    value: 'B',
        ...
        describe('remove', () => {
            ...
            it('Should remove node at index from the list when remove is called', () => {
                ...
                linkedList.addToTail('B');
                linkedList.addToTail('C');
                expect(linkedList.get(0)).excluding(`[Circualr]`).to.eql({
                    value: 'A',
                    next: {
                        value: 'B',
                        next: {
                ...
                linkedList.remove(1);
                expect(linkedList.get(0)).excluding(`[Circular]`).to.eql({
                    value: 'A',
                    next: { value: 'C', next: null, prev: `[Circular]` },
                    prev: null
                });
